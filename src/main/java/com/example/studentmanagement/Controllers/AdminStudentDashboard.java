...
public class AdminStudentDashboard implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        editButton.setOnMouseClicked(event -> {
            ...
            if (currentStudent == null) {
                ...
                alert.setHeaderText("Lỗi khi sửa thông tin học sinh");
                alert.setContentText("Vui lòng chọn học sinh cần sửa thông tin");
                alert.showAndWait();
                return;
            }
            Dialog<Void> dialog = new Dialog<>();
            ...
            dialog.setResultConverter(buttonType -> {
                if (buttonType == updateButtonType) {
                    ...
                    Integer selectedClassID = class_name.getValue() != null ? Integer.parseInt(class_name.getValue().split("-")[0]) : null;
                    Integer selectedParentID = parents.getValue() != null ? Integer.parseInt(parents.getValue().split("-")[0]) : null;
                    ...
                    String sqlUpdateStudent = "UPDATE users SET " +
                            ...
                            "student_parent_id = " + selectedParentID + " " +
                    ...
                    try {
                        ...
                        alert.setHeaderText("Sửa thông tin học sinh thành công");
                        alert.showAndWait();
                        getAndRenderStudentList();
                        dialog.close();
                    } catch (Exception e) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setTitle("Có lỗi xảy ra");
                        alert.setHeaderText("Có lỗi xảy ra khi sửa thông tin học sinh");
                        alert.setContentText(e.getMessage());
                        alert.showAndWait();
                    }
                }
